"""Cover `build_spec_for_session` rehydration of persisted session state.

This is the only path that translates AgentSession + Agent + Environment +
SessionResource + UserCredential rows into the SessionSpec shape consumed
by provisioning and turn execution. A bug here (missing field, wrong
default, type drift) silently breaks every session, so each branch of
the rehydration is pinned with a direct pytest-django test rather than
relying on slow e2e coverage.
"""
