...
def test_update_agent_not_found_returns_404(client: Client, auth_headers):
    ...
    Only ``Agent.DoesNotExist`` is reachable here — malformed UUIDs are
    intercepted by the URL converter (``<uuid:agent_id>``) before the
    view runs, so ``select_for_update().get(pk=...)`` always sees a
    valid UUID object and never raises ValueError. The catch tuple
    was narrowed to match in this PR."""
    resp = client.put(
        f"/agents/{uuid.uuid4()}",
